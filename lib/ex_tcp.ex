defmodule ExTCP do
  ...
  def handle_receive(
    ...
    case wait_segment_one_of(sock, [@psh_ack, @fin_psh_ack, @fin_ack, @fin], flow, deadline_ms) do
      {:ok, pkt, recv_flags} ->
        fin_offset = if (recv_flags &&& @fin) != 0, do: 1, else: 0
        new_server_seq = pkt.seq + byte_size(pkt.payload) + fin_offset
        send_ack(sock, src_ip, src_port, dst_ip, dst_port, my_seq, new_server_seq)

        if (recv_flags &&& @fin) != 0 and byte_size(pkt.payload) == 0 do
          {:ok, state.body, new_server_seq}
        else
